...
Всегда будет вызываться реализация `m` из класса `A`.
...
С точки зрения синтаксиса понятия интерфейса не существует.
В C++ это всего лишь абстрактный класс, содержащий только чисто виртуальные методы.
В других объектно-ориентированных языках понятие интерфейса существует, как правило,
...
- [Object Pascal: ключевое слово `interface`](https://wiki.freepascal.org/Interfaces)
...
Обратите внимание, что во всех примерах по вышеперечисленным ссылкам нет "технического бремени",
...
Более того, со временем, по мере развития программы, постепенно должна появляться поддержка новых математических функций.
...
Обратим внимание, что такую функцию можно написать и скомпилировать, не имея никакого реального класса-наследника.
Достаточно лишь наличия класса-интерфейса.
Конечно, чтобы вызвать и протестировать такую функцию, класс-наследник в конечном счёте понадобится.
```cpp
...
(на момент написания и даже компиляции самой функции `min_location`) типом, реализующим протокол `Function`.
