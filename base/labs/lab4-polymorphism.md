...
- `private`, например, сделал бы все `protected` и `public`-члены закрытыми.
...
Таким образом функции `print` можно в качестве аргумента передать любой тип, являющийся наследником от `Person`.
...
**⚠ В грамотно спроектированном коде downcast никогда использоваться не должен.**
