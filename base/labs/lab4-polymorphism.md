...
Ничто не мешает реализовать собственный наследник
...
**При этом сам оператор вывода не нужно каким-либо образом менять для поддержки нового типа данных.**
...
передачи данных по сети и т. д. в конечном счёте должен выполняться свой собственный код.
Другими словами, классам-наследникам необходим некоторый механизм, который позволял бы кастомизировать своё поведение.
...
Именно этот факт позволяет расширять поведение оператора `<<` просто за счёт добавления нового типа наследника,
никак не модифицируя при этом код самого оператора.
...
так и `std::fstream`, `std::stringstream` и любой другой наследник `std::ostream`.
...
и при этом применяется механизим раннего связывания, то говорят о **перекрытии** метода
...
решение о том, какую именно реализацию надо вызвать, принимается во время выполнения программы (поэтому и "позднее").
...
- Ключевое слово `virtual` обязательно для указания только в базовом классе.
  Данный метод становится виртуальным для всех наследников любого уровня наследования.
  В наследнике ключевое слово `virtual` указывать уже необязательно.
- Наследник не обязан переопределять виртуальный метод.
  В этом случае будет вызываться унаследованная реализация (т. е. метод из базового класса).
- Если наследник переопределяет метод, то такой метод может быть помечен ключевым словом `override`.
  ...
  что вы действительно **переопределяете** метод, то есть в базовом классе есть виртуальный метод
  с точно таким же профилем, иначе будет ошибка компиляции.
- При переопределении рекомендуется не указывать ключевое слово `virtual`,
  ...
  - Например, в базовом классе объявлен некий виртуальный константный метод.
    ...
    и если компилятор не найдёт виртуальный метод с точно такой же сигнатурой в одном из базовых классов,
...
- Переопределение метода позволяет наследнику кастомизировать своё поведение,
  ...
  не изменяя при этом код потребителя, который знает только про базовый класс
  и не знает (и не должен знать!) про фактические типы.
  - Посмотрите ещё раз на пример выше с оператором `<<`.
    ...
    в качестве левого операнда `<<`. **Менять при этом код оператора `<<` не придётся.**
...
(нет адекватной реализации по умолчанию, каждый наследник должен переопределять метод по-своему).
...
который не переопределён, то есть может не имеет никакого тела?
...
Таким образом, использование чисто виртуальных методов в базовом классе обязывает всех наследников
в конечном счёте явно переопределить все чисто виртуальные методы.

Примечание. Чисто виртуальный метод на самом деле может иметь тело,
...
При этом конкретный тип **реализует** протокол - переопределяет чисто виртуальные методы,
предоставляя некоторую реальную реализации (при этом разные типы могут предоставлять собственные реализации).

**Примечание.**
В C++ понятие интерфейса является лишь соглашением на уровне разработчиков.
С точки зрения синтаксиса понятие интерфейса не существует - это всего лишь абстрактный класс,
не содержащий никаких данных и содержательных методов.
В других языках программирования понятие интерфейса существует прямо как отдельная синтаксическая конструкция.
...
Однако мы можем ввести интерфейс для взаимодействия с такими математическими функциями,
и даже начать реализовывать какой-то код, не имея даже реальных классов-наследников.
Итак, определим интерфейс математической функции:
```cpp
class Function {
public:
...
Отметим, что содержательно такой интерфейс содержит только один метод:
- `compute` для вычисления значения функции для заданного значения аргумента `x`.
...
- другие методы, которые, по-вашему, должны быть у объекта любой математической функции.

Имея такой интерфейс, можно начать писать абстрактный код,
независящий от конкретных типов математических функций.
...
Соответсвенно, сама функция `min_location` может сделать некоторую работу, так как знает:
...
При этом при добавлении новых математических функций придётся никак менять код функции `min_location`.
Он автоматически будет работать с любым,
наперёд неизвестным (на момент написания самой функции `min_location`) типом, реализующим протокол `Function`.
...
2. Разработайте пошаговую RPG-игру, взяв за основу один из следующих вариантов:
   ...
   - текущее состояние игроков (количество жизни - в виде текста или условного progress-bar-а);
