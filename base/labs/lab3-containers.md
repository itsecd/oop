...
Демонстрация автоматического вызова деструктора для полей композитных объектов:
```cpp
#include <iostream>
...
- ⚠ Деструктор всегда вызывается автоматически.
  Вы никогда не должны вызывать его явно.
...
Однако эту идиома может применяться не только к динамической памяти, но и к любому ресурсу, который требует освобождения.
