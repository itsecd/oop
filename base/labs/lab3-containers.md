...
если не предоставлена явная реализация:
...
- конструктор - путём создания нового объекта;
- оператор - путём перезаписи уже существующего объекта.
...
В итоге в ещё живом объекте (`b`) указатель `_ptr` начинает указывать на уже недоступную память.
...
(простое копирование здесь может являться недопустимым).

Отсюда возникает следующее **правило трёх**: если в классе явно определяется один из следующих трёх методов
